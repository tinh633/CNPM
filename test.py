# Quiz Examination System (OOP + GUI) - Warehouse Templates + Published Exams + Delete + Export
# Fixes:
# - Fix Student login issue by normalizing roles in datastore load (migrate old json).
# Features:
# - Teacher: delete template, delete exam, delete attempts for an exam
# - Export: Word (.docx) for template/exam/attempt, Excel (.xlsx) for exam results
...
class TeacherFrame(ttk.Frame):
    def __init__(self, parent, app: App):
        ...
        self.exam_list = tk.Listbox(rf, height=7)
        ...
        self.attempt_list = tk.Listbox(rf, height=9)
        ...
        self.attempt_list.bind("<Button-1>", self._on_attempt_click)  # force select when click
        self.attempt_list.bind("<Double-Button-1>", lambda e: self.view_attempt_details())
    ...
    def _on_attempt_click(self, event):
        # click vào đâu -> tự chọn dòng gần nhất
...
class ExamTakeFrame(ttk.Frame):
    def __init__(self, parent, app: App):
        ...
        Header(self, "Take Exam", "System will auto submit if time is over.").pack(fill="x", pady=(0, 10))

        top = ttk.Frame(self)
        top.pack(fill="x")
        self.timer_label = ttk.Label(top, text="Time left: --:--", font=("Segoe UI", 12, "bold"))
        self.timer_label.pack(side="left")
        ttk.Button(top, text="Back", command=self.back).pack(side="right")

        body = ttk.Frame(self, padding=10)
        body.pack(fill="both", expand=True)

        self.title_label = ttk.Label(body, text="", font=("Segoe UI", 14, "bold"))
        self.title_label.pack(anchor="w", pady=(0, 8))

        self.q_label = ttk.Label(body, text="", wraplength=980, justify="left")
        self.q_label.pack(anchor="w", pady=(0, 10))

        self.opt_vars = [tk.IntVar(value=0) for _ in range(4)]
        self.check_buttons: List[ttk.Checkbutton] = []
        for i in range(4):
            cb = ttk.Checkbutton(body, text="", variable=self.opt_vars[i])
        ...
        ttk.Button(nav, text="Save", command=self.save_answer).pack(side="right", padx=6)
        ...
        ttk.Button(nav, text="Submit", command=self.submit).pack(side="right", padx=6)
    ...
    def _save_current(self):
        if not self.exam:
            return
        self.answers[self.index] = self._current_selection()

    def render(self):
        if not self.exam:
            return
        q = self.exam.questions[self.index]
        mins = max(1, self.exam.duration_seconds // 60)
        self.title_label.config(text=f"{self.exam.title} | code:{self.exam.access_code} | {mins} min")
        self.q_label.config(text=f"Q{self.index+1}: {q.text}")
        for i in range(4):
            self.check_buttons[i].config(text=q.options[i])
        saved = self.answers[self.index]
        for i in range(4):
            self.opt_vars[i].set(1 if i in saved else 0)
        self.progress_label.config(text=f"Progress: {self.index+1}/{len(self.exam.questions)}")
    ...
    def submit(self):
        self._save_current()
        if not messagebox.askyesno("Confirm", "Submit now?"):
            return
        self._submit_internal(auto=False)

    def _submit_internal(self, auto: bool):
        if not self.exam or not self.app.current_user:
            return

        submitted_at = time.time()
        time_taken = int(max(0, submitted_at - self.started_at))
        ...
        a = Attempt(
            attempt_id=self.app.store.new_attempt_id(),
            exam_id=self.exam.exam_id,
            code=self.exam.access_code,
            title=self.exam.title,
            username=self.app.current_user.username,
            full_name=self.app.current_user.full_name,
            student_id=self.app.current_user.student_id,
            score=total_score,
            total=total_q,
            started_at=self.started_at,
            submitted_at=submitted_at,
            time_taken_seconds=time_taken,
            answers=[sorted(list(s)) for s in self.answers]
        )
        ...
        if auto:
            info(f"Time is over. Auto submit.\nScore: {score10:.2f}/10")
        else:
            info(f"Submit done.\nScore: {score10:.2f}/10")
        self.back()
